...
namespace Opsive.UltimateInventorySystem.UI.Item.ItemViewModules
{
    ...
    public class ItemShapeItemView : ItemViewModule, IViewModuleMovable
    {
        ...
        protected virtual void ResizeToItemShape(ItemInfo info)
        {
            ...
            // m_ShapeResizableContent.anchoredPosition = new Vector2(-itemShape.Anchor.x * CellSize.x, itemShape.Anchor.y * CellSize.y);
            m_ShapeResizableContent.sizeDelta = new Vector2(CellSize.x                 * itemShape.Cols, CellSize.y     * itemShape.Rows);
            SetRotation(info);
        }

        public void SetRotation(ItemInfo info, bool animate = false)
        {
            // Test1
            if (info.Item == null || info.Item.TryGetAttributeValue<ItemShape>(m_ShapeAttributeName, out var itemShape) == false) {
                m_ShapeResizableContent.rotation = Quaternion.identity;
                return;
            }
            var targetRotation = Quaternion.Euler(0, 0, info.Rotation);
            var offset = info.Rotation switch{
                90  => (0, -itemShape.Cols),
                180 => (itemShape.Cols, -itemShape.Rows),
                270 => (itemShape.Rows, 0),
                _   => (0, 0)
            };
            
            var targetPos = new Vector2(-itemShape.Anchor.x * CellSize.x, itemShape.Anchor.y * CellSize.y) 
                            + new Vector2(offset.Item1      * CellSize.x, offset.Item2       * CellSize.y)
                            + m_FullOffset;
            if (animate)
            {
                m_ShapeResizableContent.DOKill();
                var animDuration = 0.3f;
                m_ShapeResizableContent.DOLocalMove(targetPos, animDuration).SetEase(Ease.OutCubic);
                m_ShapeResizableContent.DORotateQuaternion(targetRotation, animDuration).SetEase(Ease.OutCubic);
            }
            else
            {
                m_ShapeResizableContent.anchoredPosition = targetPos;
                m_ShapeResizableContent.rotation = targetRotation;
            }
        ...
        public virtual void DiscreteOffsetImage(Vector2Int offset)
        {
            ...
            m_FullOffset = new Vector2(offset.x, -offset.y) * CellSize;
        }
