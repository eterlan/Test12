...
namespace Opsive.UltimateInventorySystem.UI.Grid
{
    ...
    public class ItemShapeGridData : MonoBehaviour
    {
        ...
        // TEST1 TODO
        public GridElementData[,] m_ItemStackAnchorGrid;
        ...
        public virtual bool TryPlaceItemToPosition(ItemInfo info, Vector2Int position)
        {
            ...
            // test1 在检测处，根据旋转角度替换row 和 col
            var rows = info.IsTransposed ? shape.Cols : shape.Rows;
            var cols = info.IsTransposed ? shape.Rows : shape.Cols;
            
            
            // Out of range
            if (anchorWithOffset.x < 0 || anchorWithOffset.x + cols > GridColumns) { return false; }
            if (anchorWithOffset.y < 0 || anchorWithOffset.y + rows > GridRows) { return false; }
            ...
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < cols; col++) {
                    ...
                    if (!gridElementData.IsOccupied || !shape.IsIndexOccupied(col, row, info.IsTransposed)) { continue; }
            ...
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < cols; col++) {
                    if (shape.IsIndexOccupied(col, row, info.IsTransposed)) {
        ...
        public virtual bool IsPositionAvailable(ItemInfo info, Vector2Int position, Func<Vector2Int, bool> canIgnore = null)
        {
            ...
            // Out of range
            if (x - shape.Anchor.x < 0 || x - shape.Anchor.x + cols > GridColumns) { return false; }
            if (y - shape.Anchor.y < 0 || y - shape.Anchor.y + rows > GridRows) { return false; }

            // Check if the item fits
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < cols; col++) {

                    if (!shape.IsIndexOccupied(col, row, info.IsTransposed)) { continue; }
            ...
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < cols; col++) {
                    ...
                    if (m_ItemStackAnchorGrid[localX, localY].IsOccupied   &&
                        shape.IsIndexOccupied(col, row, info.IsTransposed) &&
        ...
        public virtual bool RemoveItemFromPosition(Vector2Int position)
        {
            ...
            // test1 在检测处，根据旋转角度替换row 和 col。特别注意，由于在拖动物品时，preview检测是否有物品和drop的时候都会移除原物品，因此此时应该使用prevRotation
            var info = m_ItemStackAnchorGrid[x, y].ItemInfo;
            var rows = info.PrevIsTransposed ? shape.Cols : shape.Rows;
            var cols = info.PrevIsTransposed ? shape.Rows : shape.Cols;
            
            // Remove item from each grid element from the shape.
            for (int row = 0; row < rows; row++) {
                for (int col = 0; col < cols; col++) {
                    if (shape.IsIndexOccupied(col, row, info.PrevIsTransposed)) {
        ...
        public virtual bool TryMoveIndex(Vector2Int sourcePos, Vector2Int destinationPos)
        {
            ...
            // Test1 增加判定条件，避免跳过检测：即使两者是同一个ItemStack，也必须旋转相同的情况才能跳过检测
            var sameRotation = destinationElement.ItemInfo.IsTransposed == sourceElement.ItemInfo.PrevIsTransposed;
            
            var isOneWayMove = (destinationElement.ItemStack == sourceElement.ItemStack && sameRotation || destinationElement.IsEmpty);
