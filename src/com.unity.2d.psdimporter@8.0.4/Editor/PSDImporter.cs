...
namespace UnityEditor.U2D.PSD
{
    ...
    public partial class PSDImporter : ScriptedImporter, ISpriteEditorDataProvider
    {
        ...
        // test2
        [SerializeField]
        private bool m_SpriteNameInvert;
...
#pragma warning disable 169, 414
        ...
        {
            var path = ctx.assetPath.Split('/');
            m_PsdFileName = path[path.Length - 1].Split('.')[0];
        ...
        private void SetCustomPivotFromGuide(Document doc)
        {
            var guides = doc.GuidesInfoGuides;
            if (guides.Length != 2)
            {
                Debug.LogWarning($"参考线数量为{guides.Length}, 应该为两条, 请检查PSB文件");
                return;
            }

            if (guides[0].Direction == guides[1].Direction)
            {
                Debug.LogWarning("两条参考线的方向一致, 请检查PSB文件");
                return;
            }
            
            // pivot需要从画布像素尺寸转换到百分比
            ...
            for (var i = 0; i < guides.Length; i++)
            {
                ...
                {
                    case 0:
                        pivot.x = guides[i].Pos / doc.width;
                        break;
                    case 1:
                        pivot.y = 1 - guides[i].Pos / doc.height; // ps是从上到下的, 换到unity得反过来
                        break;
                }
            }

            m_TextureImporterSettings.spriteAlignment = (int)SpriteAlignment.Custom;
            m_TextureImporterSettings.spritePivot = pivot;
        }
        ...
        TextureGenerationOutput ImportFromLayers(AssetImportContext ctx)
        {
            ...
            try
            {
                ...
                {
                    var index = m_SpriteNameInvert ? spriteImportData.Count - i - 1 : i;
                    spriteImportData[i].name = $"{m_PsdFileName}_{index.ToString()}";
                }
